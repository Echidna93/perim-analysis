...
subsetTime<-function(deployDat, samplePeriod=14, deployDateCol, imgDateCol){
  for(i in 1:nrow(deployDat)){
    ...
    if(all(deployDat$improperDateFormat==FALSE)){
...
propForestDat[which(propForestDat$sid == farms[j]),]$propForest1k <- propForestSml
propForestDat[which(propForestDat$sid == farms[j]),]$propForest3k <- propForestMed
propForestDat[which(propForestDat$sid == farms[j]),]$propForest5k <- propForestLrg
}

# write.table(propForestDat, "./data/propForestDat")
# propForestDat<-read.table("./data/propForestDat")
...
imgDat <- imgDat %>% filter(!(spcs %in% c("cottontail", " grey squirrel",  "grey squirrel"))) %>% mutate(spcs = str_replace_all(spcs, c("great horned owl" = "great-horned owl", "raccooon" = "raccoon", "hawk"="unknown hawk sp.", "grey-squirrel"="squirrel", "grey squirrrel" = "squirrel","striped skunk" = "skunk", "grey squirrel" = "squirrel", "groundhog; cottontail" = "groundhog", "fox squirrrel"="squirrel", "fox squirrel"="squirrel", "13 lined groundsquirrel" = "squirrel", "sp. unknown" = "sp unknown"))) %>% filter(!(spcs%in% c("squirrel", "cottontail", "great-horned owl", "unkown hawk sp.", "chipmunk", "groundhog", "cow", "sp unkown"="sp unknown"))) %>% filter(!(class== "deer" & loc== "I")) %>% filter(class!="bird") %>%  filter(class!="small-mam")
...
imgDat <- merge(imgDat, camDat, all.X=TRUE, all.y=TRUE, on.x=c('deployString'), on.y=c('deployString'))


imgDat <- splitJPGS(imgDat, "imgID")
imgDat$imgID <- toupper(imgDat$imgID)
feeders <- imgDat %>% filter(feeder==TRUE)
...
imgDat <- imgDat %>% filter(feeder==FALSE)
imgDat$uSeq <- paste0(imgDat$deployString,"-", imgDat$sequenceID)
imgDat <- imgDat %>% filter(sequenceID !=0)
write.table(imgDat, './data/imgDatClean')
write.table(feeders,'./data/feeders')
read.table("./data/feeders")
# merge with sites
sites$season<-toupper(sites$season)
sites$site<-toupper(sites$site)
sites$cam<-toupper(sites$cam)
sites$imgID <- paste0(sites$season, "-", sites$site, "-", sites$cam, "-", sites$imgNum)
imgDat2 <- imgDat %>% filter(!is.na(numInds))
imgDatm <- merge(sites, imgDat, on.x="imgID", on.y="imgID", all.y=TRUE)
```
...
imgDatUSeq <- imgDat %>% filter(!duplicated(uSeq))
imgDatUSeq <- imgDatUSeq %>% filter(!duplicated(imgDatUSeq$useq))

imgDatUSeqSummary <- imgDatUSeq %>% group_by(class) %>% summarise(n=n())
imgDatUSeqSummarySpcs <- imgDatUSeq  %>% group_by(spcs) %>% summarise(n = n(), prop=n()/nrow(imgDatUSeq))
...
imgDat$useq <- paste0(imgDat$deployString, "-", imgDat$sequenceID)
imgDatDeer <- imgDat %>% filter(class=="deer" & deerOnBothSides==TRUE) %>% filter(inSamplePeriod==TRUE)
...
for(i in 1:length(deployStrings)){
  ...
  sites.m <- sites.m %>% filter(deployString!=deployStrings[i]) # remove them
...
imgDatUSeq$imgIDsplit <- paste0(imgDatUSeq$season, "-", imgDatUSeq$site, "-", imgDatUSeq$cam, "-", imgDatUSeq$imgNum)
sites$site <- toupper(sites$site)
sites$season <- toupper(sites$season)
sites$uid <- paste0(sites$season, "-", sites$site, "-", sites$cam, "-", sites$imgNum)
sites$uid <- as.character(sites$uid)
...
mergedImgUSeq$useq <- paste0(mergedImgUSeq$deployString, "-", mergedImgUSeq$sequenceID)
mergedImgUSeq <- mergedImgUSeq %>% filter(!duplicated(useq))
...
# filter out unique sequence IDsdes
designMat <- expand.grid(nobs=0,class=c("deer", "furbearer"), deployString=deployStrings)
designMatbinFur <- expand.grid(nobs=0,class=c("deer", "otherFurbearer", "nonObScavenger"), deployString=deployStrings)
designMat <- as.data.frame(designMat)
designMat2 <- as.data.frame(designMat2)
designMat$deployString <- as.character(designMat$deployString)
designMat2$deployString <-as.character(designMat2$deployString)
designMat$class <- as.character(designMat$class)
designMat2$class <- as.character(designMat2$class)
mergedImgUSeq2 <- mergedImgUSeq %>% group_by(deployString,class) %>% filter(!duplicated(sequenceID))
# expand grid on class and deployString
mergedImgUSeq2<-data.frame(count=mergedImgDat$numInds)
designMat2$sid <- ""
designMat2$season <- ""
designMat2$cid <- ""
for(i in 1:nrow(designMat2)){
  deployString <- designMat2[i,]$deployString
  class <- designMat2[i,]$class
  if(deployString %in% mergedImgUSeq$deployString){
  designMat2[i,]$nobs <- nrow(mergedImgUSeq[which(mergedImgUSeq$deployString==deployString & mergedImgUSeq$class==class),])
  }
  designMat2[i,]$season <- strsplit(designMat2[i,]$deployString,"-")[[1]][1]
  designMat2[i,]$sid <- strsplit(designMat2[i,]$deployString,"-")[[1]][2]
  designMat2[i,]$cid <- strsplit(designMat2[i,]$deployString,"-")[[1]][3]
}
designMat2$camAngle <- 0
designMat2$camHeight <- 0
designMat2$numHours <- 0
designMat2$bearing  <- 0
designMat2$wildDeerDens <- 0
designMat2$propForest1k <- 0  
designMat2$propForest3k<- 0 
designMat2$propForest5k <- 0
designMat2$numAdults <- 0
designMat2$wildDeerDens <- 0
designMat2$sizeSqKM <- 0
designMat2$type <- ""
designMat2$facilitySizeSqMi <- 0
designMat2$lsm_l_ai_1k <- 0
designMat2$lsm_l_ai_3k <- 0
designMat2$lsm_l_ai_5k <- 0
designMat2$contactSeen <- 0

# create blank design matrix
...
designMat$cam <- ""
designMat <- as.data.frame(designMat)
mergedImgUSeq <- mergedImgUSeq[-c(which(mergedImgUSeq$class=="vague ID")),]
...
designMat2$sid <- ""
for(i in 1:nrow(designMat2)){
season<-strsplit(designMat2[i,]$deployString, "-")[[1]][1]
site<-strsplit(designMat2[i,]$deployString, "-")[[1]][2]
cam<-strsplit(designMat2[i,]$deployString, "-")[[1]][3]
designMat2[i,]$season <- season
designMat2[i,]$sid <- site
designMat2[i,]$cid <- cam
}
...
designMat<-read.table("./data/designMat")
...
#write.table(designMatF,"./data/designMatF") # save tables
...
siteSummaryObs<- designMat2S %>% group_by(deployString, sid, class) %>% summarise(encounterRate = nobs/numHours, ".groups" = "drop") %>% pivot_wider(id_cols = "sid")
...
```{r models for deer}
nbModD.1<-glmer.nb(nobs ~ camHeight.sc + propAg3k.sc + numHours.sc + (1|sid),data=designMatD)
summary(nbModD.1)
...
summary(nbModD.3)
nbModD.4<-glm.nb(nobs ~ camHeight.sc + lsm_l_shdi_5k.sc + lsm_l_ai_5k.sc + numHours.sc + propAg5k.sc + camEffort.sc,data=designMatD)
summary(nbModD.4)
performance::check_model(nbModD.4)
...
summary(nbModD.6)
performance::check_model(nbModD.6)
summary(nbModD.6)

nbModD.7<-glm.nb(nobs ~ camHeight.sc + lsm_l_shdi_5k.sc + numHours.sc + propAg5k.sc,data=designMatD)
performance::check_model(nbModD.7)

nbModD.forest<-glm.nb(nobs ~ camHeight.sc + propForest1k.sc+ lsm_l_shdi_5k.sc + numHours.sc + propAg5k.sc,data=designMatD)
performance::check_model(nbModD.forest)
nbModD.forest2<-glm.nb(nobs ~ camHeight.sc + propForest1k.sc + lsm_l_shdi_5k.sc + numHours.sc + propAg3k.sc,data=designMatD)
performance::check_model(nbModD.forest)

nbModD.forestDev<-glm.nb(nobs ~ camHeight.sc + propForest1k.sc+ lsm_l_shdi_5k.sc + numHours.sc + propForest1k.sc:lsm_l_ai_5k + propAg5k.sc,data=designMatD)
summary(nbModD.forestDev)
performance::check_model(nbModD.forestDev)

nbModD.forest.effort<-glm.nb(nobs ~ camHeight.sc + propForest1k.sc+ lsm_l_shdi_5k.sc + propAg5k.sc + numCam.sc + numHours.sc,data=designMatD)
...
nbModF.1.shdi<-glm.nb(nobs ~ numCam.sc + lsm_l_shdi_5k.sc + propAg3k.sc + numHours.sc,data=designMatF)
nbModF.1.shdi.height<-glm.nb(nobs ~ numCam.sc + camHeight.sc + lsm_l_shdi_5k.sc + propAg3k.sc+ numHours.sc,data=designMatF)
nbModF.1.ai<-glm.nb(nobs ~ numCam.sc + camHeight.sc + lsm_l_shdi_5k.sc + propAg3k.sc + numHours.sc,data=designMatF) # seem to respond to agriculture at 3km scale
performance::check_model(nbModF.1.ai)
avPlots(nbModF.1.ai)

nbModF.ai.5k<-glm.nb(nobs ~ lsm_l_ai_5k.sc + propAg3k.sc+ numHours.sc,data=designMatF)
AIC(nbModF.1.shdi,nbModF.ai.5k)

nbModF.shdi.5k<-glm.nb(nobs ~ lsm_l_shdi_5k.sc + propAg3k.sc+ numHours.sc,data=designMatF)
AIC(nbModF.1.shdi,nbModF.ai.5k,nbModF.shdi.5k)
nbModF.shdi.5k.poly<-glm.nb(nobs ~propAg3k.sc+ numHours.sc,data=designMatF)
AIC(nbModF.1.shdi,nbModF.ai.5k,nbModF.shdi.5k,nbModF.shdi.5k.poly)
nbModF.ai.5k.poly.3<-glm.nb(nobs ~ poly(lsm_l_ai_5k.sc,2) + propAg3k.sc+ numHours.sc,data=designMatF)
AIC(nbModF.1.shdi,nbModF.ai.5k,nbModF.shdi.5k,nbModF.shdi.5k.poly)

nbModF.shdi.5k.int<-glm.nb(nobs ~ propAg3k.sc+ numHours.sc,data=designMatF)
AIC(nbModF.1.shdi,nbModF.1.shdi.forest ,nbModF.ai.5k,nbModF.shdi.5k,nbModF.shdi.5k.int)
performance::check_model(nbModF.shdi.5k.int)
crPlots(nbModF.shdi.5k.int)

performance::check_model(nbModF.1.shdi.height)
performance::check_model(nbModF.1.shdi)

sum(residuals(nbModF.1.shdi.height))
sum(residuals(nbModF.1.shdi))
```
```{r}

# ggplot(data = designMatSF,
#       aes(x = (camHeight + wildDeerDens +  captiveDeerDens + offset(log(numHours)) + lsm_l_ai_1k + lsm_l_ai_3k + lsm_l_ai_5k), y = count, color = class)) + geom_point() + geom_line(aes(y=p.nbModF))
# 
# ggplot(data = designMatSD,
#       aes(x = (camHeight + wildDeerDens +  captiveDeerDens + offset(log(numHours)) + lsm_l_ai_1k + lsm_l_ai_3k + lsm_l_ai_5k),y = count, color = class)) + geom_point() + geom_line(aes(y=p.nbModD))

# PLOT FOR prediction if desired
# ggplot(data = designMat2SF,
#       aes(x = (camHeight + wildDeerDens +  captiveDeerDens + log(numHours+1) + lsm_l_ai_1k + lsm_l_ai_3k + lsm_l_ai_5k + propForest1k + propForest3k + propForest5k + numCam), y = nobs, color = class)) + geom_point() + geom_line(aes(y=p.nbModF))
```
